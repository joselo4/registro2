...
await build({ entryPoints: tests.map(name => `tests/${name}`), outdir: 'scratch/view-tests', outExtension: { '.js': '.mjs' }, bundle: true, platform: 'node', format: 'esm', packages: 'external', jsx: 'automatic', loader: { '.css': 'empty' }, define: { 'import.meta.env': '{}' } });
