...
async function publicCatalog(env, fetchImpl) {
  ...
  endpoint.searchParams.set('key', 'in.(flavors,packs,popsicles,bases,toppings,liter_config,delivery_fee,free_delivery_threshold,store_phone)');
...
function staticContent({ heading, intro, sections, deliveryText, phone, productName = '', status = true }) {
  ...
  const whatsapp = /^51\d{9}$/.test(cleanPhone) ? `https://wa.me/${cleanPhone}?text=${encodeURIComponent(message)}` : `https://wa.me/51989466466?text=${encodeURIComponent(message)}`;
  return `<main class="friozo-startup" aria-labelledby="startup-title"><header class="static-header"><a href="/" aria-label="Friozo, inicio"><img src="/logo.webp" alt="" width="48" height="48"><strong>FRIOZO</strong></a><nav aria-label="Categorías"><a href="/helados/">Helados</a><a href="/paletas/">Paletas</a><a href="/promociones/">Packs</a></nav></header><section class="static-hero"><span>HELADOS ARTESANALES EN ANDAHUAYLAS</span><h1 id="startup-title">${escapeHtml(heading)}</h1><p>${escapeHtml(intro)}</p><p class="static-delivery">${escapeHtml(deliveryText)}</p><div class="static-actions"><a class="static-primary" href="/#catalog">Explorar la carta</a><a href="${escapeHtml(whatsapp)}" rel="noopener noreferrer">Pedir por WhatsApp</a></div></section>${sections.map((section, index) => `<section class="static-section"${index === 0 ? ' id="catalog"' : ''}><h2><a href="${escapeHtml(section.path)}">${escapeHtml(section.label)}</a></h2><p>${escapeHtml(section.intro)}</p><div class="static-grid">${section.items.map(item => productCard(item, section.kind)).join('')}</div></section>`).join('')}<footer class="static-footer"><p>Los precios y la disponibilidad se confirman al completar el pedido.</p>${status ? '<p id="startup-status" role="status">La compra interactiva se está activando. Puedes ver la carta y pedir por WhatsApp.</p><button id="startup-retry" type="button" hidden>Volver a cargar la tienda</button>' : ''}</footer></main>`;
}
...
export async function prerenderStorefront(directory = 'dist', options = {}) {
  ...
  const bootstrap = Object.fromEntries(['flavors', 'packs', 'popsicles', 'bases', 'toppings', 'liter_config', 'delivery_fee', 'free_delivery_threshold', 'store_phone'].filter(key => Object.hasOwn(catalog, key)).map(key => [key, catalog[key]]));
  const pages = [{ path: '/', title: 'Friozo | Helados Artesanales, Paletas y Delivery en Andahuaylas', description: 'Elige helados artesanales, paletas y packs en Andahuaylas. Compra en línea o pide por WhatsApp.', heading: 'La carta de FRIOZO', intro: 'Elige tus favoritos y arma tu pedido en minutos.', sections }];
  ...
  for (const page of pages) {
    ...
    page.content = staticContent({ heading: page.heading, intro: page.intro, sections: page.sections, deliveryText, phone, productName: page.productName });
