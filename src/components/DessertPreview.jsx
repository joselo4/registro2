...
export function BasePhoto({base, className = ''}) {
  ...
  return <svg className={`base-photo ${className}`} viewBox={visual.crop} role="img" aria-label={base?.name || 'Envase'}><image href={customizerAsset(visual.key)} width="1280" height="1280" /></svg>;
}
...
export function ToppingPhoto({ topping, className = '' }) {
  ...
  return <svg className={`topping-photo ${className}`} viewBox={`${cell * 510} 125 510 510`} aria-hidden="true"><image href={customizerAsset('toppings-artisan')} width="2039" height="771" /></svg>;
}
// Luminance in, flavour tone curve out (see flavorTone).
function ScoopTint({ id, flavor }) {
  const [r, g, b] = flavorTone(flavor);
  return <filter id={id} colorInterpolationFilters="sRGB"><feColorMatrix type="matrix" values=".2126 .7152 .0722 0 0 .2126 .7152 .0722 0 0 .2126 .7152 .0722 0 0 0 0 0 1 0" /><feComponentTransfer><feFuncR type="table" tableValues={r} /><feFuncG type="table" tableValues={g} /><feFuncB type="table" tableValues={b} /></feComponentTransfer></filter>;
}

// Real catalogue photos when the shop uploaded one; otherwise a tinted scoop.
export function ScoopPhoto({ flavor, className = '', preferPhoto = true }) {
  const uid = useId().replace(/:/g, '');
  if (preferPhoto && flavor?.image) return <img className={`scoop-photo scoop-photo-real ${className}`} src={flavor.image} alt="" loading="lazy" decoding="async" />;
  return <svg className={`scoop-photo ${className}`} viewBox="60 60 1134 1134" aria-hidden="true"><defs><ScoopTint id={`${uid}-tint`} flavor={flavor} /></defs><image href={customizerAsset('gelato-scoop-neutral')} width="1254" height="1254" filter={`url(#${uid}-tint)`} /></svg>;
}
export default function DessertPreview({base, scoops = [], toppings = [], syrup = null, compact = false}) {
  ...
  return <svg className={`dessert-preview ${compact ? 'compact' : ''}`} viewBox={`${compact ? 35 : 0} ${top} ${compact ? 250 : 320} ${bottom-top}`} role="img" aria-label={`${visibleScoops.map(s => s.name || 'Helado').join(', ') || 'Envase vacío'} en ${base?.name || 'cono'}${toppings.length ? `. Extras: ${toppings.map(t => t.name || 'Topping').join(', ')}` : ''}${syrup ? `. ${syrup.name || 'Salsa'}` : ''}`}>
    <defs>
      ...
      {visibleScoops.map((scoop, i) => <ScoopTint key={i} id={`${uid}-flavor-${i}`} flavor={scoop} />)}
    </defs>
