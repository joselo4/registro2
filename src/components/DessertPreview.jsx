...
export function BasePhoto({base, className = ''}) {
  ...
  return <svg className={`base-photo ${className}`} viewBox={visual.crop} role="img" aria-label={base?.name || 'Envase'}><image href={`/customizer/${visual.key}.webp`} width="1280" height="1280" /></svg>;
}
...
export function ToppingPhoto({ topping, className = '' }) {
  ...
  return <svg className={`topping-photo ${className}`} viewBox={`${cell * 510} 125 510 510`} aria-hidden="true"><image href="/customizer/toppings-artisan.webp" width="2039" height="771" /></svg>;
}
export function ScoopPhoto({ flavor, className = '' }) {
  return <span className={`scoop-photo ${className}`} style={{'--flavor-color': flavorColor(flavor)}}><img src="/customizer/gelato-scoop-neutral.webp" alt="" /><span aria-hidden="true" /></span>;
}
export default function DessertPreview({base, scoops = [], toppings = [], syrup = null, compact = false}) {
  ...
  const container = <svg x={b.x} y={b.y} width={b.w} height={b.h} viewBox={b.crop} preserveAspectRatio="none"><image href={`/customizer/${b.key}.webp`} width="1280" height="1280" /></svg>;
  return <svg className={`dessert-preview ${compact ? 'compact' : ''}`} viewBox={`${compact ? 35 : 0} ${top} ${compact ? 250 : 320} ${bottom-top}`} role="img" aria-label={`${visibleScoops.map(s => s.name || 'Helado').join(', ') || 'Envase vacío'} en ${base?.name || 'cono'}${toppings.length ? `. Extras: ${toppings.map(t => t.name || 'Topping').join(', ')}` : ''}${syrup ? `. ${syrup.name || 'Salsa'}` : ''}`}>
    ...
    {visibleScoops.map((scoop, i) => {
      ...
      return <g key={`${scoop.id}-${i}`}>
        <image x={c.x-size/2} y={c.y-size/2} width={size} height={size} href="/customizer/gelato-scoop-neutral.webp" filter={`url(#${uid}-flavor-${i})`} />
        ...
        {extras.map((topping, ti) => {
          ...
          return topping.image ? <image key={topping.id} href={topping.image} x={x} y={y} width={width} height={width*.72} /> : <svg key={topping.id} x={x} y={y} width={width} height={width*.78} viewBox={`${cell*510} 125 510 510`} preserveAspectRatio="none"><image href="/customizer/toppings-artisan.webp" width="2039" height="771" /></svg>;
        })}
      </g>;
    })}
    </g>
