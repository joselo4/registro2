...
export default function OperationsCenter({orders=[],groups=[],salesGoal=0,onNavigate,onUpdateOrderStatus,shopOpen}) {
  ...
  async function advanceOrder(order) {
    if (pending) return;
    setPending(order.id);
    const nextStatus = order.status === 'Por Corroborar' ? 'Pendiente' : (order.status === 'Pendiente' ? 'Preparando' : 'En camino');
    try {
      const synced = await onUpdateOrderStatus(order.id, nextStatus);
      setFeedback(synced === false ? `Pedido ${order.id} actualizado localmente. La sincronización sigue pendiente.` : `Pedido ${order.id}: estado actualizado a ${nextStatus}.`);
    } catch {
      setFeedback('No se pudo actualizar el pedido. Intenta nuevamente.');
    } finally {
      setPending(null);
    }
  }
