...
export default function CartItemPreview({ item, literConfig }) {
  ...
  return <div className="cart-product-preview">{src ? <img src={src} alt={item.name || 'Producto'} /> : <span aria-hidden="true">{item.type === 'liter' ? '🏺' : item.type === 'popsicle' ? '🍭' : item.type === 'extra' ? (item.icon || '🧁') : '🎁'}</span>}</div>;
}
