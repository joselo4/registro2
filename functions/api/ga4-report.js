import { fail, getAuthenticatedUser, isTrustedAdmin, json, normalizeEmail, sameOriginRequest } from './_security.js';
...
export async function onRequestGet({ request, env }, dependencies = {}) {
  try {
    ...
    let allowed = isTrustedAdmin(auth.user);
    if (!allowed) {
      const { data, error } = await auth.adminClient.from('helados_sync').select('value').eq('key', 'staff_users').maybeSingle();
      if (error) return fail(502, 'authz', 'No se pudo verificar el rol del usuario.');
      allowed = Array.isArray(data?.value) && data.value.some(staff => normalizeEmail(staff.email) === normalizeEmail(auth.user.email) && String(staff.role || '').toLowerCase().includes('admin') && !String(staff.status || '').toLowerCase().includes('suspend'));
    }
    if (!allowed) return fail(403, 'authz', 'Solo un administrador puede ver las conversiones.');
