...
test('API GA4 exige administrador y configuración del servidor', async () => {
  ...
  const admin = { user: { email: 'admin@donhelado.com', app_metadata: { role: 'Administrador' } } };
...
test('API GA4 consulta solo eventos del embudo con credenciales del servidor', async () => {
  ...
  const response = await onRequestGet({ request: new Request('https://example.com/api/ga4-report?days=7'), env: { GA4_PROPERTY_ID: '123456', GA4_SERVICE_ACCOUNT_JSON: JSON.stringify(credentials) } }, { authenticate: async () => ({ user: { email: 'admin@donhelado.com', app_metadata: { role: 'Administrador' } } }), fetcher });
